...
- **Web dev server runs the full Next.js server** (`next dev`, port 3100). Dev uses `apps/web/.next-dev/` (`NEXT_DIST_DIR=.next-dev`); production Pages builds use `apps/web/.next/` + `EIP_STATIC_EXPORT=1` (set by the web `build` script). That separation stops `next build` from corrupting a running local app. Do **not** set `NODE_ENV=production` in the parent shell — use `npm run build:web` / `npm run build -w @ellines-eip/web` instead. Auth is client-side (JWT in `localStorage`); the web app calls the identity API at `NEXT_PUBLIC_API_URL` (`http://localhost:3001` locally; production uses same-origin Pages Functions unless overridden).
