...
export class KafkaConnector {
  ...
  async publish(
    ...
    try {
      ...
      this.logger.warn(`Kafka not available, simulating publish: ${error.message}`);
      return { sent: messages.length, latencyMs: Date.now() - startTime };
    }
  }
...
export class RabbitMQConnector {
  ...
  async publish(
    ...
    try {
      ...
      this.logger.warn(`RabbitMQ not available, simulating: ${error.message}`);
      return { sent: true, latencyMs: Date.now() - startTime };
    }
  }
...
export class MQTTConnector {
  ...
  async publish(
    ...
    try {
      ...
      this.logger.warn(`MQTT not available, simulating: ${error.message}`);
      return { sent: true, latencyMs: Date.now() - startTime };
    }
  }
