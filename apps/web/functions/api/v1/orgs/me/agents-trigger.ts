/**
 * Pages Function: POST /api/v1/orgs/me/agents-trigger
 *
 * Execution engine entry point: fire an enterprise event → find matching
 * active agents → evaluate conditions → score confidence → create executions.
 *
 * Called by:
 *   - The event bus (on enterprise events)
 *   - Manual test from the automation UI
 *   - Connector sync callbacks
 *
 * Owner/IT Admin only.
 */
import {
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  if (!eventType) {
    return json({ statusCode: 400, message: 'eventType is required' }, 400);
  }
  ...
  const agents: AgentRecord[] = ((settings.ellineaAgents as AgentRecord[] | undefined) ?? [])
    .filter((a) => a && a.isActive && !a.isPaused && a.trigger === eventType);

  if (!agents.length) {
    return json({ triggered: 0, executions: [], message: `No active agents for event "${eventType}"` });
  }

  const ctx = { ...asObj(body.payload), eventType };
  const newExecutions: ExecutionRecord[] = [];
  const updatedAgents = [...((settings.ellineaAgents as AgentRecord[]) ?? [])];
  ...
  for (const agent of agents) {
    ...
    const execution: ExecutionRecord = {
      ...
      confidence: score,
      reasoning: `Action "${agent.action?.type}" scored ${(score * 100).toFixed(0)}% confidence.`,
  ...
  if (!newExecutions.length) {
    return json({ triggered: 0, executions: [], message: 'No conditions matched' });
  }
