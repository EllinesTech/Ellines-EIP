...
import {
  ...
  activatePlatformOrgConnector, deactivatePlatformOrgConnector,
...
const [pkg,setPkg]=useState({name:'',displayName:'',maxUsers:25,maxConnectors:5,requestsPerDay:10000,monthlyPrice:0,enableSso:false,enableCustomRoles:false,enableAgents:false,enableAdvancedBi:false,enableWebhooks:false});
 ...
 const load=useCallback(async()=>{try{
  ...
  useEffect(() => {
    ...
    setWsTab('overview');
    setWsLoading(true);
    // Each fetch is wrapped individually so a single failing endpoint never
    // wipes the entire workspace (e.g. a 500 on agents must not clear connectors).
    Promise.all([
      fetchPlatformOrgStats(clientOrgId).catch(() => null),
      listPlatformOrgUsers(clientOrgId).catch(() => [] as import('@/lib/api').OrgMember[]),
      fetchPlatformOrgPackage(clientOrgId).catch(() => null),
      fetchPlatformOrgDateTimeSettings(clientOrgId).catch(() => ({timeFormat:'24h',dateStyle:'medium'} as import('@/lib/api').OrgDateTimeSettingsDto)),
      fetchPlatformOrgConnectors(clientOrgId).catch(() => [] as PlatformOrgConnectorDto[]),
      fetchPlatformOrgApprovals(clientOrgId).catch(() => [] as PlatformOrgApprovalDto[]),
      fetchPlatformOrgRules(clientOrgId).catch(() => [] as PlatformOrgRuleDto[]),
      fetchPlatformOrgReports(clientOrgId).catch(() => [] as PlatformOrgReportDto[]),
      fetchPlatformOrgAgents(clientOrgId).catch(() => [] as PlatformOrgAgentDto[]),
      fetchPlatformOrgSnapshot(clientOrgId).catch(() => null as PlatformOrgSnapshotDto),
      fetchPlatformOrgConnectorInstallations(clientOrgId).catch(() => [] as ConnectorInstallationDto[]),
      listPlatformOrgDocuments(clientOrgId).catch(() => [] as PlatformDocumentDto[]),
      fetchPlatformOrgProfile(clientOrgId).catch(() => null as PlatformOrgProfileDto | null),
      ...
      if(s !== null) setWsStats(s);
      setWsUsers((u ?? []) as import('@/lib/api').OrgMember[]);
      if(t !== null) setWsTier(t);
      setWsSettings((d ?? {timeFormat:'24h',dateStyle:'medium'}) as import('@/lib/api').OrgDateTimeSettingsDto);
      setWsConnectors((conn ?? []) as PlatformOrgConnectorDto[]);
      setWsApprovals((appr ?? []) as PlatformOrgApprovalDto[]);
      setWsRules((rules ?? []) as PlatformOrgRuleDto[]);
      setWsReports((reports ?? []) as PlatformOrgReportDto[]);
      setWsAgents((agents ?? []) as PlatformOrgAgentDto[]);
      setWsSnapshot(snap as PlatformOrgSnapshotDto);
      setWsInstallations((inst ?? []) as ConnectorInstallationDto[]);
      setWsDocuments((docs ?? []) as PlatformDocumentDto[]);
      setWsOrgProfile(profile as PlatformOrgProfileDto | null);
      setWsIntegrationRequests((intReqs ?? []) as IntegrationRequestDto[]);
    }).catch(e=>{
      // Only fires if Promise.all itself throws (shouldn't happen with safe()),
      // but guard against auth failures anyway.
  ...
  function resolveContent() {
    ...
    switch (activeSection) {
      ...
      case 'client':           return <ClientWorkspace
          ...
          onActivateConnector={async (connId) => {
            if (!clientOrgId) return;
...
function ClientWorkspace({
  ...
  onActivateConnector: (connId:string) => Promise<void>;
  onDeactivateConnector: (connId:string) => Promise<void>;
  ...
  const [wizSystemLabel, setWizSystemLabel] = useState('');
  ...
  function resetWizard() {
    setWizStep(1); setWizCatalogId('openapi'); setWizDisplayName(''); setWizSystemLabel(''); setWizEditingId(null); setWizPackId('');
  ...
  function buildWizConfig(): import('@/lib/api').ConnectorInstallConfigDto {
    const c: import('@/lib/api').ConnectorInstallConfigDto = { authType: wizAuthType, systemName: wizDisplayName||undefined, systemLabel: wizSystemLabel||undefined };
  ...
  function editInstallation(inst: import('@/lib/api').ConnectorInstallationDto) {
    resetWizard();
    setWizEditingId(inst.id); setWizCatalogId(inst.catalogId); setWizDisplayName(inst.displayName); setWizSystemLabel(String(inst.config?.systemLabel||'')); setWizPackId(inst.packId||'');
    const c=inst.config||{};
            ...
            <div style={{marginBottom:10,padding:'8px 14px',borderRadius:10,background:'rgba(124,58,237,.10)',border:'1px solid rgba(124,58,237,.2)',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
              ...
              {tier.rate_limit_tiers.max_connectors !== null &&
               installations.filter(c=>c.status!=='deleted').length >= tier.rate_limit_tiers.max_connectors && (
                <span style={{fontSize:11,color:'#fb7185',fontWeight:700}}>âš  Limit reached — upgrade package to add more</span>
              )}
            </div>
          )}
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}>
            <div className={styles.grid4} style={{gap:8}}>
              ...
              <Kpi label="Active" value={installations.filter(c=>c.status==='active'||c.status==='synced').length} hint="enabled for client" cls={styles.ok}/>
              ...
              <Kpi label="Draft / Suspended" value={installations.filter(c=>c.status==='draft'||c.status==='suspended').length} hint="inactive"/>
            </div>
